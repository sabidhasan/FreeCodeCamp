...
$(document).ready(function() {
  ...
  $(".nav-link").on("click", function() { showContent($(this).attr("data-target")); });
});
...
function showContent(currId) {
  ...
  $(".nav-link").removeClass("active slideInLeft");
  document.getElementsByClassName("nav-link")[active].className += " slideInLeft active";
}

$(window).bind("DOMMouseScroll mousewheel", function(e) {
  active =
    e.originalEvent.wheelDelta / 120 > 0 ? (active - 1) % navs.length : (active + 1) % navs.length;
